...
public class ServiceMorbidity2100 extends Service {
    ...
    public ServiceMorbidity2100(int year, int region, String baseFolder, String suffixForm) throws OncoException {
        ...
        String extVariant = null;
        if (year < 2011) {
            extVariant = ".xls";
        } else {
            extVariant = "_np.xls";
        }
        String nameOfFileForParsing = sourceFile.getParent() + "\\" + sourceFile.getName().substring(0, (sourceFile.getName().length() - 4)) + extVariant;
        ...
        if(year == 2005){
            sheetNameOfSourceFile = "форма";
        }else{
            sheetNameOfSourceFile = "форма1";
        }
    }
